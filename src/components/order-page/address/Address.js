    ...
    <Card className="ml-3 mr-3 mt-2 border-0">
      <Card.Body className="p-2">
        <Card.Title className="align-middle xtBlack">
          ...
          {specificOrder ? specificOrder.name : order?.address.name}
        </Card.Title>
        <Card.Subtitle className="mb-2 text-muted d-flex justify-content-between">
          ...
          ${
            ...
            : `${order?.address.doorNo} , ${order?.address.area} , ${
                order?.address.street !== undefined ? order?.address.street : ''
              }
          ${
            order?.address.landmark !== undefined ? order?.address.landmark : ''
          } ${order?.address.city}`}
        </Card.Subtitle>
      </Card.Body>
    </Card>
